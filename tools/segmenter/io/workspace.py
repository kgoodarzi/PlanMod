...
class WorkspaceManager:
    ...
    def _deserialize_mask_regions(self, data: list, image: np.ndarray, region_type: str) -> list:
        """Deserialize mask regions and recreate masks."""
        if image is None:
            return []
        ...
        for r in data:
            ...
            # Recreate mask based on region type
            mask = np.zeros((h, w), dtype=np.uint8)
            
            if region.get('mode') == 'auto':
                # For auto regions, recreate from bbox
                ...
                if 'mask_rle' in region and 'mask_bbox' in region:
                    ...
                    try:
                        ...
                        print(f"Error decoding mask RLE: {e}")
                        import traceback
                        traceback.print_exc()
                else:
    ...
    def _deserialize_object(self,
        ...
        for inst_data in data.get("instances", []):
            ...
            # Get image for this instance's page
